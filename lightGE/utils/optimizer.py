...
class Adam(Optimizer):
    ...
    def __init__(self, parameters, lr=0.001, beta=(0.9, 0.999), eps=1e-8):
        ...
        self.t = 0
        for p in self.parameters:
            self.m.append(Tensor(np.zeros(p.shape)).to(p.device))
            self.v.append(Tensor(np.zeros(p.shape)).to(p.device))
    ...
    def step(self, zero=True):
        ...
        for p, m, v in zip(self.parameters, self.m, self.v):
            if p.device == "cpu":
                ...
                m.data = self.beta[0] * m.data + (1 - self.beta[0]) * cp.array(p.grad)
                v.data = self.beta[1] * v.data + (1 - self.beta[1]) * (cp.array(p.grad) ** 2)

                m_hat = m.data / (1 - self.beta[0] ** self.t)
                v_hat = v.data / (1 - self.beta[1] ** self.t)
                p.data -= cp.array(self.lr * m_hat / (cp.sqrt(v_hat) + self.eps))
    ...
    def step(self, zero=True):
        ...
        for p, m in zip(self.parameters, self.m):
            ...
            if p.device == "cpu":
                ...
                p.data -= self.lr * p.grad / (cp.sqrt(m.data) + self.eps)
